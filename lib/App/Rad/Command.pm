...
ALIAS_CHECK: # try to find if user given an alias instead
    ...
    # al newkirk (awnstudio): arg option prompt support
    if ( defined $self->{args}->{$arg_real_name}->{prompt}) {
        unless ( $val ) {
            my $ask = $self->{args}->{$arg_real_name}->{prompt};
            my $set = ( defined $self->{args}->{$arg_real_name}->{default}
                       ? $self->{args}->{$arg_real_name}->{default} : '' );
            
            my $opt = $arg_real_name;
    ...
    return ($arg_real_name, undef, $val);
}
